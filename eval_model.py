...
def eval_model(model, dataset):
    ...
    n_batches = round(len(dataset)/(batch_size*n_check)) # how many total batches in the data set
    ...
    for i, (images, labels) in enumerate(data_loader, 1):
        ...
        images = images.to(device) # get current batch images
    ...
    print("Final Validation Accuracy:", round(correct/len(dataset), 4)*100, "%")
...
def main():
    ...
    model = AutoModelForImageClassification.from_pretrained('block_pruning_results/final_finetuned/')
    # model = torch.load('./DynamicViT/logs/dynamicvit_deit-b/checkpoint-best.pth', map_location='cpu')
    
    valid_dataset = ImageFolder(root='/nfs/hpc/share/buivy/pruning-vision-transformers/val', 
                                transform=transform)

    eval_model(model, valid_dataset) # run model through validation set (dset)
